...
pub mod cqt {
	...
	// Sample at the centre of data
	// what if there aren't enough samples though?
	// eh, panic
	// we also don't care about phase...
	...
	pub fn naive_cqt(root_freq: f64, bin: u8, rate: f64, resolution: u8, data: &[i16]) -> u16 {
		...
		for j in left_bound..right_bound {
			let real_common = (data[j] as f64) * (1.0 / N_k) * window((right_bound - left_bound), j);
			let complex_common = exp_complex((j as i16 + (N_k.floor() as i16)/2 ) - (data.len()/2) as i16, f_k, rate);		
...
mod tests {
	...
	fn special_values(){
		...
		assert_eq!(N_k_precalc.round(), cqt::calc_N_k(bin, rate, resolution, f_k).round());
	...
	fn waves(){
		...
		*/
		...
		for i in 0..22050 {
			...
			for j in 0..3 {
				...
				addend = ((32768.0 / 3.0) * (2.0 * ((i as f64 * freq / rate) - (0.5 + (i as f64 * freq / rate)).floor())));
		...
		*/
		...
		let mut cb_start = [0_i16, 0_i16, 0_i16, 0_i16, 0_i16, 0_i16, 0_i16, 0_i16, 0_i16, 0_i16];
		for i in 0..10 {
		...
		for i in 0..48 {
			...
			println!("{}\t{}", cqt::calc_f_k(root_freq, i as u8, resolution).round(), c);
		}
